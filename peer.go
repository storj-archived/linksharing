...
// Peer is the representation of a Linksharing itself.
//
// architecture: Peer
type Peer struct {
	Log      *zap.Logger
...
func New(log *zap.Logger, config Config) (_ *Peer, err error) {
	...
	service, err := sharing.NewService(log, peer.Mapper)
	if err != nil {
		return nil, err
	}
	...
	peer.Server, err = httpserver.New(log, peer.Listener, service, config.Server)
	if err != nil {
		return nil, err
	}
...
// Run runs linksharing until it's either closed or it errors.
func (peer *Peer) Run(ctx context.Context) error {
	group, ctx := errgroup.WithContext(ctx)

	// start linksharing as a separate goroutine.
	group.Go(func() error {
