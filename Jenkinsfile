    ...
    stages {
        ...
        stage('Verification') {
            parallel {
                ...
                stage('Testsuite') {
                    environment {
                        STORJ_COCKROACH_TEST = 'cockroach://root@localhost:26257/testcockroachtestsuite?sslmode=disable'
                        STORJ_POSTGRES_TEST = 'postgres://postgres@localhost/teststorjtestsuite?sslmode=disable'
                        COVERFLAGS = "${ env.BRANCH_NAME != 'master' ? '' : '-coverprofile=../.build/coverprofile -coverpkg=./...'}"
                    }
                    steps {
